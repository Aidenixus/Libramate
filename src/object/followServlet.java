...
public class followServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		roster r = t.getR();
		String next = "/profileOthersUnfollow.jsp";
		String followedname = request.getParameter("followedname");
		String followername = request.getParameter("followername");
		User followed = r.getUserByUsername(followedname);
		User follower = r.getUserByUsername(followername);
		follower.addFollowing(followedname);
		followed.addFollower(followername);
		r.update(follower);
		r.update(followed);
		...
		t.save(actual);
		User user = r.getUserByUsername(followedname);
